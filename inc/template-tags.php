...
if ( ! function_exists( 'cosparell_posts_navigation' ) ) :
/**
 * Display navigation to next/previous set of posts when applicable.
 *
 * @todo Remove this function when WordPress 4.3 is released.
 */
function cosparell_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Posts navigation', 'cosparell' ); ?></h2>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( __( 'Older posts', 'cosparell' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts', 'cosparell' ) ); ?></div>
			<?php endif; ?>
...
if ( ! function_exists( 'cosparell_post_navigation' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 *
 * @todo Remove this function when WordPress 4.3 is released.
 */
function cosparell_post_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Post navigation', 'cosparell' ); ?></h2>
		<div class="nav-links">
			<?php
...
function cosparell_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		// /* commented out to remove the posted in tab in posts heading
		// translators: used between list items, there is a space after the comma */
		// $categories_list = get_the_category_list( __( ', ', 'cosparell' ) );
		// if ( $categories_list && cosparell_categorized_blog() ) {
		// 	printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'cosparell' ) . '</span>', $categories_list );
		// }

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'cosparell' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'cosparell' ) . '</span>', $tags_list );
		}
	}
	...
	// edit_post_link( __( 'Edit', 'cosparell' ), '<span class="edit-link">', '</span>' );
}
endif;
...
function cosparell_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'cosparell_categories' ) ) ) {
